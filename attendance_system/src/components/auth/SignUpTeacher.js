...
class SignUpTeacher extends Component {
  ...
  onSubmit = (e) => {
    e.preventDefault();
    if (this.validate()) {
      const { email, password, department, fname, lname } = this.state;
      this.setState({
        loading: true,
      });
      firebase
        ...
        .then((res) => {
          ...
          this.props.firestore
            .set(
              ...
              { email, status: "NV", department, name: name }
            )
            .then(() => {
              //console.log("test");

              this.props.firestore
                .set({ collection: "users", doc: uid }, { type: "teacher" })
                .then(() => {
...
export default compose(
  ...
  firestoreConnect([{ collection: "teachers" }, { collection: "users" }])
)(SignUpTeacher);
